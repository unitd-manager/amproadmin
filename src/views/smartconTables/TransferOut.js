...
const Test = () => {
  ...
  const getSupplier = () => {
    ...
    api
      .post('/salesOrder/getsalesorder', {
        ...
        from_date: fromDate,
        to_date: toDate,
        customer: FromLocationFilter,
        customers: ToLocationFilter,
        status: statusFilter,
      })
      .then((res) => {
  ...
  const columns = [
    ...
    { name: 'From Location', selector: 'company_name', sortable: true, grow: 0, wrap: true },
    { name: 'To Location', selector: 'printed', sortable: true, grow: 0, wrap: true },
    { name: 'Status', selector: 'status', sortable: true, grow: 0, wrap: true },
    { name: 'Remark', selector: 'sub_total', sortable: true, grow: 0, wrap: true },
    { name: 'Has Discrepancy', selector: 'tax', sortable: true, grow: 0, wrap: true },
    { name: 'Discrepancy Remark', selector: 'net_total', sortable: true, grow: 0, wrap: true },
...
useEffect(() => {
      ...
      <div className="pt-xs-25">
        ...
        <div className="d-flex flex-wrap gap-2 mb-3">
          ...
          <select className="form-select"   style={{ width: '25%' }}
           value={FromLocationFilter} onChange={(e) => setFromLocationFilter(e.target.value)}>
            <option value="">Select From Location</option>
            <option value="Head Office">Head Office</option>
          </select>
          <select className="form-select"   style={{ width: '25%' }}
           value={ToLocationFilter} onChange={(e) => setToLocationFilter(e.target.value)}>
          ...
          Button={
            <Dropdown isOpen={dropdownOpen} toggle={toggleDropdown}>
              <DropdownToggle color="primary" caret className="shadow-none">
                <Button color="primary" tag={Link} to="/TransferOutDetails" className="shadow-none">
                  New Transaction
                </Button>
              </DropdownToggle>
              <DropdownMenu>
  ...
  {element.status !== 'Closed' ? (
    <Link to={`/TransferOutEdit/${element.sales_order_id}`}>
      <Icon.Edit2 />
    </Link>
  ) : (
    ''
