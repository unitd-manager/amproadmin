      ...
      prevRows.map(row => {
        if (row.pd_product_id === id) {
          ...
          if (field === "discount_percentage") {
            ...
            updatedRow.discount_amount = Number(((qty * price * discountPercentage) / 100).toFixed(2));
          }
...
updatedRow.total_price = Number(grossTotal.toFixed(2));
  ...
  const deleteRow = (index,id) => {
    if(id){
      api.post('/purchaseorder/deletePdProduct',{pd_product_id:id}).then(() => {
...
export default PurchaseDebitNoteEdit;
