...
useEffect(() => {
  ...
  const handleProductSelect = (index, selectedProduct) => {
        ...
        let piecesPerCarton = Number(base?.pcs_per_carton || base?.pieces_per_carton || base?.carton_qty || 0);
        if (!piecesPerCarton) {
          const unitStr = base?.unit || base?.UOM || base?.unit_name || '';
        ...
        const unitPrice = Number(base?.wholesale_price || base?.unit_price || base?.price || 0);
        ...
        const cartonPrice = Number(base?.carton_price || (unitPrice && piecesPerCarton ? unitPrice * piecesPerCarton : 0));
        ...
        // compute totals using existing carton/loose/qty values
        ...
        const uomLabel = base?.unit || base?.UOM || base?.unit_name || `${piecesPerCarton}`;
...
console.log('formdata',payloadForm);
    api
    .post('/purchaseorder/editPurchaseInvoice', payloadForm)
    .then(() => {
      const purchaseOrderId = id;
      rows?.forEach((el) => {
        if (!el.product_id) return;
        el.gross_total = el.total_price;
        const isNewRow = String(el.pi_product_id || '').startsWith('new-');
        if (isNewRow) {
          const insertPayload = { ...el, purchase_invoice_id: purchaseOrderId };
          api.post('/purchaseorder/insertPiProduct', insertPayload).then(() => {});
        } else {
          api.post('/purchaseorder/editPiProduct', el).then(() => {});
        }
      });
  ...
  const addRow = (insertAfterIndex) => {
    ...
    const newRow = {
      pi_product_id: `new-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
      product_code: "",
      product_name: "",
      carton_qty: 0,
      loose_qty: 0,
  ...
  const handleDelete = (index,id) => {
    const updatedRows = rows.filter((row) => row.pi_product_id !== id);
    setRows(updatedRows);
    deleteRow(index,id);
  };
  useEffect(() => {
        ...
        <tbody>
          {rows.map((p, idx) => (
            <React.Fragment key={p.pi_product_id}>
              <tr key={p.pi_product_id} style={{ fontSize: '13px', height: '20px', background:  '#fff' }}>
                <td
                  style={{
                    padding: '0.3rem',
                    cursor: 'pointer',
              ...
              />
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    bsSize="sm"
                    value={p.carton_qty}
                    onChange={(e) => handleRowChange(p.pi_product_id, 'carton_qty', e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    bsSize="sm"
                    value={p?.loose_qty}
                    onChange={(e) => handleRowChange(p.pi_product_id, 'loose_qty', e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onChange={(e) => handleRowChange(p.pi_product_id, 'carton_price', e.target.value)}
                    style={{ width: '80px' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onChange={(e) => handleRowChange(p.pi_product_id, 'price', e.target.value)}
                    style={{ width: '80px' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <div className="d-flex" >
                    <Input
                      ...
                      onChange={(e) => handleRowChange(p.pi_product_id, 'discount_percentage', e.target.value)}
                    ...
                    <Input
                      ...
                      onChange={(e) => handleRowChange(p.pi_product_id, 'discount_amount', e.target.value)}
                      style={{ width: '50%' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onChange={(e) => handleRowChange(p.pi_product_id, 'grossTotal',e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <td style={{ padding: '0.3rem', whiteSpace: 'nowrap' }}>
                  <Button
                    ...
                    color="danger"
                    className="me-1"
                    onClick={() => handleDelete(idx,p.pi_product_id)}
