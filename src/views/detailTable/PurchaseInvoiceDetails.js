...
const handleKeyDown = (e) => {
  ...
  const addNewRow = (callback) => {
    let newPoProductId;
    setRows((prevRows) => {
      newPoProductId = prevRows.length > 0 ? Math.max(...prevRows.map(r => r.pi_product_id)) + 1 : 1;
      const updatedRows = [
        ...prevRows,
        {
          pi_product_id: newPoProductId,
          product_code: "",
          product_name: "",
          carton_qty: 0,
          loose_qty: 0,
...
console.log('formData', payloadForm);
    ...
    try {
      const res = await api.post('/purchaseorder/insertPurchaseInvoice', payloadForm);
  ...
  const generateCode = () => {
      return api
        .post('/commonApi/getCodeValues', { type: 'purchaseInvoice' })
        .then((res) => {
