...
const handleKeyDown = (e) => {
  ...
  const addNewRow = (callback) => {
    let newPoProductId;
    setRows((prevRows) => {
      newPoProductId = prevRows.length > 0 ? Math.max(...prevRows.map(r => r.gr_product_id)) + 1 : 1;
      const updatedRows = [
        ...prevRows,
        {
          gr_product_id: newPoProductId,
          product_code: "",
          product_name: "",
          carton_qty: 0,
          loose_qty: 0,
...
console.log('formData', payloadForm);
    ...
    try {
      const res = await api.post('/purchaseorder/insertGoodsReceipt', payloadForm);
  ...
  const generateCode = () => {
      return api
        .post('/commonApi/getCodeValues', { type: 'goodsReceipt' })
        .then((res) => {
 ...
 // Handle product code selection
  ...
  const handleDelete = (index,id) => {
    const updatedRows = rows.filter((row) => row.gr_product_id !== id);
    setRows(updatedRows);
    deleteRow(index,id);
  };
  return (
        ...
        <tbody>
          {rows.map((p, idx) => (
            <React.Fragment key={p.gr_product_id}>
              <tr key={p.gr_product_id} style={{ fontSize: '13px', height: '20px', background:  '#fff' }}>
                <td
                  style={{
                    padding: '0.3rem',
                    cursor: 'pointer',
              ...
              />
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    bsSize="sm"
                    value={p.carton_qty}
                    onChange={(e) => handleRowChange(p.gr_product_id, 'carton_qty', e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    bsSize="sm"
                    value={p.loose_qty}
                    onChange={(e) => handleRowChange(p.gr_product_id, 'loose_qty', e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onChange={(e) => handleRowChange(p.gr_product_id, 'carton_price', e.target.value)}
                    style={{ width: '80px' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    bsSize="sm"
                    value={p?.price}
                    onChange={(e) => handleRowChange(p.gr_product_id, 'price', e.target.value)}
                    style={{ width: '80px' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onChange={(e) => handleRowChange(p.gr_product_id, 'total', e.target.value)}
                    style={{ width: '80px' }}
                    readOnly
                  />
                </td>
                <td style={{ padding: '0.3rem' }}>
                  <div className="d-flex" >
                    <Input
                      ...
                      onChange={(e) => handleRowChange(p.gr_product_id, 'discount_percentage', e.target.value)}
                    ...
                    <Input
                      ...
                      onChange={(e) => handleRowChange(p.gr_product_id, 'discount_amount', e.target.value)}
                      style={{ width: '50%' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onChange={(e) => handleRowChange(p.gr_product_id, 'grossTotal',e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                ...
                <td style={{ padding: '0.3rem', whiteSpace: 'nowrap' }}>
                  <Button
                    ...
                    color="danger"
                    className="me-1"
                    onClick={() => handleDelete(idx,p.gr_product_id)}
