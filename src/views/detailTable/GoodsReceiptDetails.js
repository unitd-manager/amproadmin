...
const { id } = useParams();
 ...
 const [currency, setCurrency] = useState({
  ...
  const handleAddExtraFields = (id) => {
    setRows(rows.map(p =>
      p.gr_product_id === id ? { ...p, showExtraFields: !p.showExtraFields, remarks: p.remarks || '', foc_qty: p.foc_qty || 0 } : p
    ));
  };
...
const handleKeyDown = (e) => {
  ...
  const addNewRow = () => {
    setRows((prevRows) => [
      ...prevRows,
      {
        gr_product_id: prevRows.length > 0 ? Math.max(...prevRows.map(r => r.gr_product_id)) + 1 : 1,
        product_code: "",
        product_name: "",
        carton_qty: 0,
        loose_qty: 0,
    ...
    try {
      ...
      await Promise.all(
        rows.map(el => {
          ...
          if(el.product_id){
          return api.post('/purchaseorder/insertGrProduct', el);
          }
        })
      );

      message('Goods Receipt has been Created successfully.', 'success');
  ...
  const deleteRow = (index, id) => {
    ...
    if (rows.length > 2) {
      ...
      const clearedRows = rows.map((row, i) =>
        i === index
          ? {
              gr_product_id: row.gr_product_id,
              product_code: "",
              product_name: "",
              carton_qty: 0,
              loose_qty: 0,
      ...
      <Container fluid className="p-1 mb-5">
        {/* <Card className="shadow-sm">
          <CardBody className="p-3"> */}
            {/* Header */}
            <h6 className="mb-2">Add/Edit Goods Receipt</h6>
...
<TabPane tabId="1">
  ...
  <Row>
    ...
    <col style={{ width: "8rem" }} />  {/* Total */}
        ...
        <tbody>
          {rows?.map((p, idx) => (
            <React.Fragment key={p.gr_product_id}>
              <tr key={p.gr_product_id}>
                <td
                  style={{
                    padding: '0.3rem',
                    cursor: 'pointer',
              ...
              />
                 ...
                 <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      ...
                      handleRowChange(p.gr_product_id, 'carton_qty', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.gr_product_id, 'carton_qty', val);
                      }
                    }}
                 ...
                 <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.gr_product_id, 'loose_qty', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.gr_product_id, 'loose_qty', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                 ...
                 <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.gr_product_id, 'qty', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.gr_product_id, 'qty', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.gr_product_id, 'carton_price', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.gr_product_id, 'carton_price', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.gr_product_id, 'price', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.gr_product_id, 'price', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        const currentRowIndex = rows.findIndex(r => r.gr_product_id === p.gr_product_id);
                        const nextRowIndex = currentRowIndex + 1;
                        const nextRow = rows[nextRowIndex];
                        if (nextRow) {
                          const nextProductCodeInput = document.querySelector(
                            `input[name="product_code_${nextRow.gr_product_id}"]`
                          );
                          ...
                          setTimeout(() => {
                            ...
                            const newProductCodeInput = document.querySelector(
                              `input[name="product_code_${newRow.gr_product_id}"]`
                            );
                  ...
                  <div className="d-flex" >
                    <Input
                      ...
                      onBlur={(e) => {
                        const val = e.target.value;
                        const num = val === '' ? 0 : parseFloat(val);
                        handleRowChange(p.gr_product_id, 'discount_percentage', num);
                      }}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (/^\d*\.?\d{0,2}$/.test(val)) {
                          handleRowChange(p.gr_product_id, 'discount_percentage', val);
                        }
                      }}
                      onFocus={(e) => e.target.select()}
                      style={{ width: '50%', marginRight: '2px', textAlign: 'right' }}
                      name={`discount_percentage_${p.gr_product_id}`}
                  />
                    <Input
                      type="text"
                      bsSize="sm"
                      ...
                      onBlur={(e) => {
                        const val = e.target.value;
                        const num = val === '' ? 0 : parseFloat(val);
                        handleRowChange(p.gr_product_id, 'discount_amount', num);
                      }}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (/^\d*\.?\d{0,2}$/.test(val)) {
                          handleRowChange(p.gr_product_id, 'discount_amount', val);
                        }
                      }}
                      onFocus={(e) => e.target.select()}
                      style={{ width: '50%', textAlign: 'right' }}
                      name={`discount_amount_${p.gr_product_id}`}
                    />
                  </div>
                </td>
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    readOnly
                    style={{ width: '80px', textAlign: 'right' }}
                    name={`gross_total_${p.gr_product_id}`}
                   />
                </td>
                <td style={{ padding: '0.3rem', whiteSpace: 'nowrap' }}>
                  <Button
                    ...
                    color="danger"
                    className="me-1"
                    onClick={() => deleteRow(idx,p.gr_product_id)}
                  ...
                  >
                   ...
                   <Button
                    ...
                    color="success"
                    className="me-1"
                    onClick={() => handleAddExtraFields(p.gr_product_id)}
                    style={{ padding: '0.1rem 0.3rem', fontSize: '0.7rem' }}
                  >
                ...
                <tr>
                  ...
                  <td colSpan={1} style={{ padding: '0.3rem' }}>
                    ...
                    <Input
                      ...
                      value={p.remarks}
                      onChange={(e) => handleRowChange(p.gr_product_id, 'remarks', e.target.value)}
                    />
                  </td>
                  ...
                  <td colSpan={1} style={{ padding: '0.3rem' }}>
                    ...
                    <Input
                      ...
                      style={{ fontSize: '0.75rem', padding: '0.1rem' }}
                      value={p.foc_qty}
                      onChange={(e) => handleRowChange(p.gr_product_id, 'foc_qty', e.target.value)}
                    />
                  </td>
                  ...
                  <td colSpan={1} style={{ padding: '0.3rem' }}>
                    ...
                    <Input
                      ...
                      value={p.UOM || ''}
                      onChange={(e) => handleRowChange(p.gr_product_id, 'UOM', e.target.value)}
                    >
                      <option>Uom</option>
                  ...
                  <td colSpan={1} style={{ padding: '0.3rem' }}>
                    ...
                    <Input
                      ...
                      style={{ fontSize: '0.75rem', padding: '0.1rem' }}
                      value={p.kilo_price || ''}
                      onChange={(e) => handleRowChange(p.gr_product_id, 'kilo_price', e.target.value)}
                    />
                  </td>
                  <td colSpan={1} style={{ padding: '0.3rem' }}>
                    ...
                    <Input
                      ...
                      onChange={(e) => handleRowChange(p.gr_product_id, 'standard_rate', e.target.value)}
                    >
