              ...
              />
                 ...
                 <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      ...
                      handleRowChange(p.goods_return_product_id, 'carton_qty', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.goods_return_product_id, 'carton_qty', val);
                      }
                    }}
                 ...
                 <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.goods_return_product_id, 'loose_qty', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.goods_return_product_id, 'loose_qty', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                 ...
                 <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.goods_return_product_id, 'qty', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.goods_return_product_id, 'qty', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.goods_return_product_id, 'carton_price', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.goods_return_product_id, 'carton_price', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    onBlur={(e) => {
                      const val = e.target.value;
                      const num = val === '' ? 0 : parseFloat(val);
                      handleRowChange(p.goods_return_product_id, 'price', num);
                    }}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (/^\d*\.?\d{0,2}$/.test(val)) {
                        handleRowChange(p.goods_return_product_id, 'price', val);
                      }
                    }}
                    onFocus={(e) => e.target.select()}
                    style={{ width: '80px', textAlign: 'right' }}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        const currentRowIndex = rows.findIndex(r => r.goods_return_product_id === p.goods_return_product_id);
                        const nextRowIndex = currentRowIndex + 1;
                        const nextRow = rows[nextRowIndex];
                        if (nextRow) {
                          const nextProductCodeInput = document.querySelector(
                            `input[name="product_code_${nextRow.goods_return_product_id}"]`
                          );
                          ...
                          setTimeout(() => {
                            ...
                            const newProductCodeInput = document.querySelector(
                              `input[name="product_code_${newRow.goods_return_product_id}"]`
                            );
                ...
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    readOnly
                    style={{ width: '80px', textAlign: 'right' }}
                    name={`total_${p.goods_return_product_id}`}
                  />
                </td>
                  ...
                  <div className="d-flex" >
                    <Input
                      ...
                      onBlur={(e) => {
                        const val = e.target.value;
                        const num = val === '' ? 0 : parseFloat(val);
                        handleRowChange(p.goods_return_product_id, 'discount_percentage', num);
                      }}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (/^\d*\.?\d{0,2}$/.test(val)) {
                          handleRowChange(p.goods_return_product_id, 'discount_percentage', val);
                        }
                      }}
                      onFocus={(e) => e.target.select()}
                      style={{ width: '50%', marginRight: '2px', textAlign: 'right' }}
                      name={`discount_percentage_${p.goods_return_product_id}`}
                  />
                    <Input
                      type="text"
                      bsSize="sm"
                      ...
                      onBlur={(e) => {
                        const val = e.target.value;
                        const num = val === '' ? 0 : parseFloat(val);
                        handleRowChange(p.goods_return_product_id, 'discount_amount', num);
                      }}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (/^\d*\.?\d{0,2}$/.test(val)) {
                          handleRowChange(p.goods_return_product_id, 'discount_amount', val);
                        }
                      }}
                      onFocus={(e) => e.target.select()}
                      style={{ width: '50%', textAlign: 'right' }}
                      name={`discount_amount_${p.goods_return_product_id}`}
                    />
                  </div>
                </td>
                <td style={{ padding: '0.3rem' }}>
                  <Input
                    ...
                    readOnly
                    style={{ width: '80px', textAlign: 'right' }}
                    name={`gross_total_${p.goods_return_product_id}`}
                   />
                </td>
                <td style={{ padding: '0.3rem', whiteSpace: 'nowrap' }}>
                  <Button
                    ...
                    color="danger"
                    className="me-1"
                    onClick={() => deleteRow(idx,p.goods_return_product_id)}
