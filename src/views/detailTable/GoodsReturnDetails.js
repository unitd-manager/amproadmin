...
useEffect(() => {
  ...
  const getColumnSum = (key) => {
    ...
    const handleProductSelect = (index, selectedProduct) => {
      ...
      // find full product record from products list (options contain minimal fields)
      const prod = products.find(p => p.product_id === selectedProduct.value) || selectedProduct;

      // determine pieces per carton from various possible fields
      ...
      // prefer explicit carton/loose from product, otherwise compute from on-hand qty
      let cartonQty = 0;
      let looseQty = 0;
