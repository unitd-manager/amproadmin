...
const insertSettingData = (code) => {
 ...
 if (settingdetails.company_id !== '') {
      settingdetails.invoice_code = code;
      settingdetails.invoice_date = new Date().toISOString().slice(0, 10);
      ...
      return api
        .post('/invoice/insertInvoice', payload)
        .then((res) => {
  ...
  try {
    ...
    if (pendingSalesmen && pendingSalesmen.length > 0) {
      const savePromises = pendingSalesmen.map((salesman) =>
        api.post('/employee/addInvoiceSalesman', {
          invoice_id: newId,
