...
export default function SalesManComponent({  salesOrderId, onSavePendingSalesmen }) {
  ...
  const getSalesmenList = () => {
    if (salesOrderId) {
      api.post('/employee/getSalesOrderSalesmen', { sales_order_id: salesOrderId })
        .then((res) => {
  ...
  const addSalesman = () => {
    if (selectedSalesman) {
      ...
      if (
        ...
        api.post('/employee/addSalesOrderSalesman', data)
          .then(() => {
  ...
  const deleteSalesman = (salesId) => {
    ...
    console.log('Deleting salesman with data:', { sales_id: salesId, sales_order_id: salesOrderId });

    api.post('/employee/removeSalesOrderSalesman', {
      sales_order_id: salesOrderId,
      sales_id: salesId,
    })
      .then((res) => {
