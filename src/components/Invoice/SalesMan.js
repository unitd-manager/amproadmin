...
export default function SalesManComponent({  deliverOrderid, onSavePendingSalesmen }) {
  ...
  const getSalesmenList = () => {
    if (deliverOrderid) {
      api.post('/employee/getInvoiceSalesmen', { invoice_id: deliverOrderid })
        .then((res) => {
  ...
  const deleteSalesman = (salesId) => {
    ...
    console.log('Deleting salesman with data:', { sales_id: salesId, invoice_id: deliverOrderid });

    api.post('/employee/removeInvoiceSalesman', {
      invoice_id: deliverOrderid,
      sales_id: salesId,
    })
      .then((res) => {
