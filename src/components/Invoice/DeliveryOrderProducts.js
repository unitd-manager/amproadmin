...
import EditLineItemModal from './DelEditLineItemModal';
import QuoteLineItem from './DeliveryLineItem';
  ...
  const fetchBillDiscountAndTax = async () => {
    try {
      const response = await api.post('/invoice/getDeliveryorderById', {
        delivery_order_id: id, // Changed from delivery_order to delivery_order_id
      });
      ...
      // Only make the tax API call if we have a tax type
      if (type) {
        ...
        setTaxRate(0); // Default to 0 if no tax type
      }
    } catch (error) {
      console.error('Failed to fetch bill discount or tax info:', error);
      setBillDiscount(0); // Set defaults on error
      setTaxRate(0);
      setTaxType('');
    }
  };
...
const saveBillDiscount = async (value) => {
  try {    await api.post('/invoice/updateBillDelDiscount', {
  ...
  >
    {index + 1}
          ...
          <td>
            ...
            <span className="addline" onClick={() => deleteRecord(e.delivery_order_item_id)}>
              <Icon.Trash2 />
            </span>
          </td>
        </tr>
