    ...
    const productContent = await Promise.all(
      products.map(async (item) => {
        ...
        if (["Print With Price", "Print With Retail Price", "Print With Stock"].includes(printOption)) {
          priceDetails.push({
            text: `RPrice : $${typeof item.rprice === 'number'
              ? item.rprice.toFixed(2)
              : Number(item.rprice)
              ? Number(item.rprice).toFixed(2)
              : '0.00'}`,
            style: 'priceText',
          });
        }
        if (["Print With Price", "Print With Stock"].includes(printOption)) {
          priceDetails.push({
            text: `CPrice : $${typeof item.cprice === 'number'
              ? item.cprice.toFixed(2)
              : Number(item.cprice)
              ? Number(item.cprice).toFixed(2)
              : '0.00'}`,
            style: 'priceText',
          });
          priceDetails.push({
            text: `WPrice : $${typeof item.wprice === 'number'
              ? item.wprice.toFixed(2)
              : Number(item.wprice)
              ? Number(item.wprice).toFixed(2)
              : '0.00'}`,
            style: 'priceText',
          });
        }
