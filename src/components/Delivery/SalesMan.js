...
export default function SalesManComponent({  deliverOrderid, onSavePendingSalesmen }) {
  ...
  const getSalesmenList = () => {
    if (deliverOrderid) {
      api.post('/employee/getDvOrderSalesmen', { delivery_order_id: deliverOrderid })
        .then((res) => {
  ...
  const deleteSalesman = (salesId) => {
    ...
    if (!salesId || !deliverOrderid) {
      console.error('Missing required data:', { salesId, deliverOrderid });
      alert('Error: Missing sales ID or sales order ID');
      return;
    }

    console.log('Deleting salesman with data:', { sales_id: salesId, delivery_order_id: deliverOrderid });

    api.post('/employee/removeDvOrderSalesman', {
      delivery_order_id: deliverOrderid,
      sales_id: salesId,
    })
      .then((res) => {
