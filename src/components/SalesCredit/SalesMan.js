...
export default function SalesManComponent({  salesOrderId, onSavePendingSalesmen }) {
  ...
  const getSalesmenList = () => {
    if (salesOrderId) {
      api.post('/employee/getCreditNoteSalesmen', { credit_note_id: salesOrderId })
        .then((res) => {
  ...
  const deleteSalesman = (salesId) => {
    ...
    console.log('Deleting salesman with data:', { sales_id: salesId, credit_note_id: salesOrderId });

    api.post('/employee/removeCreditNoteSalesman', {
      credit_note_id: salesOrderId,
      sales_id: salesId,
    })
      .then((res) => {
