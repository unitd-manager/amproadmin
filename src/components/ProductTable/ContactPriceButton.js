// Full working code for ContactPriceButton with company name auto-fill on code select

import React, { useState, useEffect, useContext } from 'react';
import { Input, Button, Modal, ModalFooter,
...
export default function ContactPriceButton({ addPurchaseOrderModal, setAddPurchaseOrderModal, ProductId, productDetails }) {
  ...
  const AddNewLineItem = () => {
    setMoreItem([...addMoreItem, {
      ...
      itemId: '', unit: '', qty: '', price: '', mrp: '', gst: 0, description: '',
    }]);
  };
  ...
  const ProductLineItemById = () => {
    api.post('/product/getCSProductByProductId', { product_id: ProductId })
  ...
  const EditCSProductLineItems = () => {
    addMoreItem.forEach((item) => {
      api.post('/product/EditCSProductLineItems', item)
  ...
  const onchangeItem = (selectedOption, itemId) => {
    const updated = addMoreItem.map((el) =>
      el.id === itemId ? { ...el, itemId: selectedOption.value, title: selectedOption.company_name } : el
    );
    setMoreItem(updated);
  };
  ...
  const PurchaseRequestLineItemById = () => {
    api.post('/product/getCSSupplierProductByProductId', { product_id: ProductId })
  ...
  const editPurchaseRequestItems = () => {
    purchaserequesteditdetails.forEach((item) => {
      api.post('/product/EditCSProductLineItemsBYSupplierID', item)
        .then(() => message('Line Item Edited Successfully', 'success'))
        .catch(() => message('Cannot Edit Line Items', 'error'));
    });
  };
  ...
  const AddNewCustomerLineItem = () => {
    setPurchaseRequestEditDetails([
      ...
      { id: random.int(0, 9999).toString(), contact_id: '', name: '', wholesale_price: 0, carton_price: 0, fixed_price: 0 }
    ]);
  };
  ...
  useEffect(() => {
    if (addPurchaseOrderModal) {
      setMoreItem([
        { id: random.int(1, 99).toString(), itemId: '', unit: '', qty: '', price: '', mrp: '', gst: '', description: '' },
      ]);
    }
  ...
  return (
    <Modal size="xl" isOpen={addPurchaseOrderModal}>
      <ComponentCard>
        <Nav tabs>
          <NavItem><NavLink className={activeTab === '1' ? 'active' : ''} onClick={() => toggle('1')}>Customer</NavLink></NavItem>
          <NavItem><NavLink className={activeTab === '2' ? 'active' : ''} onClick={() => toggle('2')}>Supplier</NavLink></NavItem>
        </Nav>
        ...
        <TabContent activeTab={activeTab}>
          <TabPane tabId="1">
            ...
            <table className="lineitem">
              <thead>
                <tr><th>Code</th><th>Name</th><th>Wholesale Price</th><th>Carton Price</th><th>Fixed Price</th><th>Action</th></tr>
              </thead>
              <tbody>
                {addMoreItem.map((el, index) => (
                  <tr key={el.id}>
                    <td>
                      <Select options={getProductValue} onChange={(e) => onchangeItem(e, el.id)} />
                    </td>
                    <td><Input value={el.title || ''} readOnly /></td>
            ...
            <ModalFooter>
              <Button color="primary" onClick={EditCSProductLineItems}>Submit</Button>
              <Button color="secondary" onClick={() => setAddPurchaseOrderModal(false)}>Cancel</Button>
            </ModalFooter>
          </TabPane>

          <TabPane tabId="2">
            <Button color="primary" onClick={AddNewCustomerLineItem}>Add Item</Button>
            <table className="lineitem">
              <thead>
                <tr><th>Code</th><th>Name</th><th>Wholesale Price</th><th>Carton Price</th><th>Fixed Price</th></tr>
              </thead>
              <tbody>
                {purchaserequesteditdetails.map((item, index) => (
                  <tr key={item.id}>
                    <td>
                      <Select options={getsupplier} onChange={(e) => updateSupplierState(index, 'contact_id', { target: { value: e.value } })} />
                    </td>
                    <td><Input value={item.name} onChange={(e) => updateSupplierState(index, 'name', e)} /></td>
                    <td><Input value={item.wholesale_price} onChange={(e) => updateSupplierState(index, 'wholesale_price', e)} /></td>
                    <td><Input value={item.carton_price} onChange={(e) => updateSupplierState(index, 'carton_price', e)} /></td>
                    <td><Input value={item.fixed_price} onChange={(e) => updateSupplierState(index, 'fixed_price', e)} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
            <ModalFooter>
              <Button color="primary" onClick={editPurchaseRequestItems}>Submit</Button>
              <Button color="secondary" onClick={() => setAddPurchaseOrderModal(false)}>Cancel</Button>
            </ModalFooter>
          </TabPane>
        </TabContent>
